...
const ReviewSection = () => {
  ...
  const fetchReviews = async () => {
    setLoading(true);
    try {
      const res = await apiClient.get(`/plans/${planId}/reviews/`);
  ...
  const onSubmit = async (data) => {
    console.log(data);
    try {
      await authApiClient.post(`/plans/${planId}/reviews/`, data);
      fetchReviews();
    } catch (error) {
  ...
  const checkUserPermission = async () => {
    try {
      const res = await authApiClient.get(`/orders/has-ordered/${planId}/`);
  ...
  const handleDeleteReview = async (reviewId) => {
    try {
      await authApiClient.delete(`/plans/${planId}/reviews/${reviewId}/`);
      fetchReviews();
    } catch (error) {
      console.log(error);
    }
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="text-base-content/70">
            Be the first to review this plan!
          </p>
        </div>
      ) : (
        <ReviewList
